// components/ZplToPdfConverter.tsx
"use client";
...
export default function ZplToPdfConverter() {
  ...
  const handleNewFile = (selected: File) => {
    if (!selected.name.toLowerCase().endsWith(".zpl")) {
      setError("Please select a .zpl file.");
      resetAll();
      return;
    }
    setError("");
  ...
  const handleConvert = async () => {
    if (!file) {
      setError("Please select a ZPL file first.");
      return;
    }
    setLoading(true);
    ...
    try {
      const response = await fetch("/api/ZplConvert", {
        method: "POST",
        body: formData,
      });
        ...
        <div className="bg-white rounded-lg p-8 py-16 cursor-pointer hover:bg-gray-50 transition duration-200 flex flex-col justify-center items-center">
          {file ? (
            ...
            <p className="text-xl text-gray-500">
              Drag & drop your <span className="font-bold">.zpl</span> file here
              or click to select
            </p>
          )}
          <input
